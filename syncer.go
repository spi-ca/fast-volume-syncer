...
func syncerEntry() {
	...
	if daemonized || selectorInvoked {
		_ = common.SetProcessName("syncer")
		log.SetFlags(0)
	} else {
	...
	runner := syncer.Runner{
		...
		Source: common.RemoteInfo{
			MountInfo: common.MountInfo{
				Host:    viper.GetString("src.storage.mount.host"),
				Path:    argSrcStoragePath,
				Options: viper.GetString("src.storage.mount.option"),
			},
		...
		Destination: common.RemoteInfo{
			MountInfo: common.MountInfo{
				Host:    viper.GetString("dst.storage.mount.host"),
				Path:    argDstStoragePath,
				Options: viper.GetString("dst.storage.mount.option"),
			},
