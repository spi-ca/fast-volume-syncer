...
func (r *Runner) logVolumeInfo(ctx context.Context, path string) {
	...
	if out, err := exec.CommandContext(ctx, "ls", "-al", path).CombinedOutput(); err != nil {
		...
		prefix := fmt.Sprintf("directory_info(%s)=>\t", path)
		r.logLineByLine(bytes.NewReader(out), prefix)
	}
...
func (r *Runner) locateFindBinary() string {
	if len(r.FindBinaryPath) < 1 {
		return ""
	}

	if foundPath, err := exec.LookPath(r.FindBinaryPath); err != nil {
		log.Printf("find path(%s) not found", r.FindBinaryPath)
		return ""
	} else {
...
func (r *Runner) prepareDirectory() (string, string, error) {
	if !r.SandboxDisabled {
		if err := r.sandbox(); err != nil {
	...
	if err = os.Chdir(tempDir); err != nil {
		return "", "", fmt.Errorf("failed to change directory(%s): %w", tempDir, err)
	}
	...
	if err = r.mount(r.Source.MountInfo, srcMountPath); err != nil {
		return "", "", fmt.Errorf("mount failed(%s %s) : %w", r.Source.MountInfo, srcMountPath, err)
	}
	log.Printf("source mount success!(%s %s)", r.Source.MountInfo, srcMountPath)

	if err = r.mount(r.Destination.MountInfo, dstMountPath); err != nil {
		return "", "", fmt.Errorf("mount failed(%s %s) : %w", r.Destination.MountInfo, dstMountPath, err)
	}
	...
	if err != nil {
		return "", "", fmt.Errorf("failed to get source file info(%s): %w", srcMountSubPath, err)
	...
	return srcMountSubPath, dstMountSubPath, nil
}

func (r *Runner) Execute(ctx context.Context) error {
	// chdir 영향으로 미리 발견하여야 된다.
	findBinaryPath := r.locateFindBinary()

	srcPath, dstPath, err := r.prepareDirectory()
	if err != nil {
		return err
	}
	...
	return joiner.Execute(ctx, entryRecvChan)
}
